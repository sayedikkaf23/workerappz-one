  ...
  if (email) {
    this.svc.getOnboardingDetailsByEmail(email).subscribe(
      (data) => {
        ...
        // ✅ Handle shareholders
        this.formData.numberOfShareholders = data.data.numberOfShareholders || 1;

        if (data.data.shareholders && data.data.shareholders.length > 0) {
          this.formData.shareholders = data.data.shareholders.map((sh: any) => ({
        ...
        if (this.formData.shareholders.length < this.formData.numberOfShareholders) {
          for (let i = this.formData.shareholders.length; i < this.formData.numberOfShareholders; i++) {
        ...
        } else if (this.formData.shareholders.length > this.formData.numberOfShareholders) {
          this.formData.shareholders = this.formData.shareholders.slice(0, this.formData.numberOfShareholders);
        }
      },
      () => {
